...
ROOT=${ROOT:-./data}
GPU=${GPU:-0}
MODEL_DIR=${MODEL_DIR:-output/cifar100_ir100_clip_vitb16_adaptformer_aft_pgd2_eps1.0}
ATTACK=${ATTACK:-fgsm}
EPS=${EPS:-0.25}
ALPHA=${ALPHA:-0.25}
STEPS=${STEPS:-1}
...
run_eval() {
  ...
  python tools/eval_adv_lift.py \
    ...
    --alpha "${ALPHA}" \
    --steps "${STEPS}" \
    --out_csv "${OUT_DIR}/adaptformer_aft_pgd2_train_eps1.0_${ckpt_label}_${ATTACK}_steps${STEPS}_eps${EPS}_summary.csv" \
    --classwise_csv "${OUT_DIR}/adaptformer_aft_pgd2_train_eps1.0_${ckpt_label}_${ATTACK}_steps${STEPS}_eps${EPS}_classwise.csv" \
    adaptformer True
}
